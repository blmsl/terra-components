...
export class TerraSplitViewComponentNew implements OnDestroy, OnInit, OnChanges
{
    ...
    {
        let viewFound: TerraSplitViewDetail = this.modules.find((module) => module.identifier == view.mainComponentName);

        if(viewFound)
        {
            let hasSameParams:boolean = false;

            for(let detailView of viewFound.views)
            {
                ...
                {
                    detailView.changeDetector.detach();
                }
            }

            // TODO: set isSelected Attribute of the views to hide views in the DOM
            if(!hasSameParams)
            {
                viewFound.views.push(view);
            }
        }
        else
        {
        ...
        this.zone.runOutsideAngular(
            () =>
            {
                setTimeout(
                    function()
                    {
                        console.log('Start Execution ' + new Date() + new Date().getMilliseconds().toString());
                        let anchor = $('#' + id);
                        ...
                        breadCrumbContainer.children('li').each(
                            function()
                            {
                                var container = $(this);

                                container.find('a:not(.caret)').each(
                                    function()
                                    {
                                        $(this)
                                            .off();
                        ...
                        console.log('End Execution ' + new Date() + new Date().getMilliseconds().toString());
                    });
